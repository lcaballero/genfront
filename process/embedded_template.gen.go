...
var _dump_structFm = []byte(`{{ range $key, $val := . }}{{ $key }} {{ $val }}
{{ end }}-------------------------------------------------------------------------------
Names:
...
var _plainFm = []byte(`package {{ GOPACKAGE }}
`)

func plainFmBytes() ([]byte, error) {
	return _plainFm, nil
}

func plainFm() (*asset, error) {
	bytes, err := plainFmBytes()
	if err != nil {
		return nil, err
	}
